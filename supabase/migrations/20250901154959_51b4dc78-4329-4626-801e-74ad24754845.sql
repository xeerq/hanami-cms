-- OSTATECZNA NAPRAWA BEZPIECZEŃSTWA - Kompletny refactoring (poprawiony)
...
SELECT 
  id,
  name,
  "position",  -- Escapowanie słowa kluczowego
  bio,
  image_url,
  social_links,
  display_order,
...
RETURNS TABLE (
  id uuid,
  name text,
  position_name text,  -- Zmieniona nazwa kolumny
  bio text,
  image_url text,
  social_links jsonb,
...
BEGIN
  ...
  SELECT 
    tm.id,
    tm.name,
    tm."position" as position_name,  -- Escapowanie słowa kluczowego
    tm.bio,
    tm.image_url,
    tm.social_links,
    tm.display_order,
