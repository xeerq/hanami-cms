...
-- Create rate limiting tracking table for authentication attempts
...
-- Add session cleanup function for security
...
AS $$
BEGIN
  -- Clean up old auth rate limits older than 24 hours
  DELETE FROM public.auth_rate_limits 
  WHERE created_at < now() - interval '24 hours';
  
  -- Clean up old audit logs older than 90 days (for performance)
